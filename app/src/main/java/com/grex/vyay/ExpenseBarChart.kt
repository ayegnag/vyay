...
fun ExpenseBarChart(
    ...
    val maxIncome = incomes.maxByOrNull { it.totalAmount }?.totalAmount ?: 0f
    val maxAmount = maxOf(maxExpense, maxIncome)
    val utils = Utilities()
    ...
    Column(modifier = modifier.fillMaxWidth()) {
        Box(
            ...
            Canvas(
                ...
                expenses.forEachIndexed { index, expense ->
                    ...
                    val incomeBarHeight = (incomes.getOrNull(index)?.totalAmount ?: 0f) / maxAmount * size.height
                    ...
                    drawRect(
                        ...
                        topLeft = Offset(index * barWidth + barWidth * 0.4f, size.height - expenseBarHeight),
        ...
        Row(
            ...
            expenses.forEachIndexed { index, expense ->
                ...
                val expenseAmount = NumberFormat.getCurrencyInstance(Locale("en", "IN")).format(expense.totalAmount)
                val incomeAmount = income?.let { NumberFormat.getCurrencyInstance(Locale("en", "IN")).format(it.totalAmount) } ?: "-"
                ...
                Column(
                    ...
                    Text(
                        text = "Inc: $incomeAmount",
                        color = PatinaShine,
                        style = MaterialTheme.typography.labelSmall,
                    )
                    Text(
                        text = "Exp: $expenseAmount",
                        color = ClayBeige,
                        style = MaterialTheme.typography.labelSmall,
                    )
                }
